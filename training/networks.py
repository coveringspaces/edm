...
#----------------------------------------------------------------------------
# CFM modfication to the preconditioning proposed in the paper "Elucidating the Design
# Space of Diffusion-Based Generative Models" (EDM).

@persistence.persistent_class
class CFMPrecond(torch.nn.Module):
    def __init__(self,
        img_resolution,                     # Image resolution.
        img_channels,                       # Number of color channels.
        label_dim       = 0,                # Number of class labels, 0 = unconditional.
    ...
    def forward(self, x, t, class_labels=None, force_fp32=False, return_logvar=False, **model_kwargs):
        ...
        # outout from the model that we are pretending is v since that is what cfm trains
        v = self.model((c_in * x).to(dtype), c_noise.flatten(), class_labels=class_labels, **model_kwargs)
        v = v.to(torch.float32)

        alpha = torch.cos(t)  # cos(t)
        s     = torch.sin(t)  # sin(t)
